...
A reverse code review: review the _developer_, not the code. Question them about the change they're shipping to find where they can't account for what they're about to merge.
...
**Line numbers drift; don't cite from memory.** A diff's line numbers shift the moment anything above them changes: a later edit, a commit, a rebase. Never quote a line number you're recalling from earlier in the conversation. Re-derive it from the _current_ file at the moment you cite it, and verify the line number and what you intend to call out match.

**Pin the base yourself, don't assume one.** A diff needs a base to be a diff against — the repo's default branch, a PR's declared base, or whatever the user names. If it isn't obvious, ask; don't default to `develop`, `main`, or any other fixed branch name.
...
A good question can't be answered by reading the code aloud but by understanding _why_.
...
> "Which part of this diff do you understand _least_? If it's wrong, how would you find out: review, CI, prod, or perhaps never?"
...
- Attestation clean, nothing left to account for → **rt-pr-description**
- Gaps found, but they're gaps in the *code* — a missing guard, an untested edge case → back to **rt-address-findings**
- Gaps found because the *plan itself* was ambiguous or wrong, and no amount of re-reading the diff would resolve it → say so explicitly, and send it back to **rt-interview** or **rt-to-tickets** instead of another round of code changes

## Tone

Curious, not prosecutorial: an ally catching the thing that would have come back to bite them later. The promise: _don't imitate the agent's output, understand it well enough to be accountable for it._
